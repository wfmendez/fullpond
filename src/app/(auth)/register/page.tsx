...
export default function RegisterPage() {
  const [state, action, pending] = useActionState<AuthState | undefined, FormData>(registerAction, undefined);
  ...
  const labelCls = "mb-1.5 block text-xs font-semibold uppercase tracking-wider text-fp-dark/60";

  return (
    <div className="rounded-3xl bg-white/90 p-8 shadow-xl backdrop-blur-sm border-2 border-fp-dark/10">
      <h1 className="font-display text-3xl tracking-tight text-fp-dark">Create your account</h1>
      <p className="mt-1 text-sm text-fp-dark/60">Apply to FullPond&apos;s open roles.</p>
      ...
      <form action={action} className="space-y-4">
        ...
        <div>
          ...
          <input id="country" name="country" type="text" autoComplete="country-name"
            placeholder="e.g. Mexico" className={inputCls} />
        </div>
        <div>
      ...
      <p className="mt-6 text-center text-sm text-fp-dark/60">
        ...
        <Link href="/login" className="font-semibold text-fp-dark hover:underline">
          Sign in
        </Link>
      </p>
    </div>
